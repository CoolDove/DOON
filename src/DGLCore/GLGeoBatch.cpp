...
GeoBatch::GeoBatch(std::initializer_list<Attribute> _attribs) {
	...
	glVertexArrayElementBuffer(vao_, elem_buf_.get_id());
...
void GeoBatch::upload() {
	...
	elem_buf_.allocate(elem_size, BufferFlag::DYNAMIC_STORAGE_BIT | BufferFlag::MAP_READ_BIT);
	elem_buf_.upload_data(elem_size, 0, indices_.data());
}
